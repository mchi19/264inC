...
/** 
 * Compute length of C string 'str', excluding the terminating null byte ('\0')
 * 
 * Examples:
 * my_strlen("abc"); // 3
 * my_strlen("");   // 0
...
/**
 * Return a pointer that points to the first occurrence of 
 * character 'ch' in C string 'str'. Return NULL if 'ch' is not found.
 *
 ...
 *
 * Please read the README before attempting this function.
 */
char * my_strchr(const char * str, int ch);

/** 
 * Same as my_strchr(...), except it searches from the right-hand-side
 * (in other words, it returns a pointer to the last occurence)
 *
 * Examples:
 * const char * str = "Hello World!";
...
/**
 * Append (concatenate) C-string 'src' to C-string 'dest'. A precondition is 'dest' must be 
